...
export default function SearchClient({ metas }: Props) {
  ...
  const categories = useMemo(() => {
    return Array.from(new Set(metas.map((m) => m.category))).slice(0, 8)
  }, [metas])
    ...
    <Wrap>
      ...
      {!filtered.length ? (
        <EmptyState role="status" aria-live="polite">
          ...
          <EmptyText>
            Versuch’s mit kürzeren Begriffen oder wähle eine Abkürzung unten.
          </EmptyText>
          {!!topTags.length && (
            <>
          ...
          <EmptyLinks>
            ...
            <Link href="/tags/over%209000">#over 9000</Link>
          </EmptyLinks>
        </EmptyState>
      ) : (
        <>
